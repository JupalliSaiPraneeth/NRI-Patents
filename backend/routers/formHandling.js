...
const DB_COLUMN_MAP = {
  ...
  grantDocumentLink: 'grantdocumentlink',
};
      ...
      .insert([
        {
          ...
          department,
          caste: caste || null,
          coapplicants: coApplicants || null,
          patentid: patentId,
          patenttitle: patentTitle,
          patenttype: finalPatentType,
          approvaltype: approvalType || 'Published',
          filingdate: formatDateForMySQL(filingDate),
          grantingdate: formatDateForMySQL(grantingDate),
          publishingdate: formatDateForMySQL(publishingDate),
          documentlink: finalDocumentLink,
          grantdocumentlink: finalGrantDocLink,
          authors
        }
      ]);
        ...
        .insert([
          {
            ...
            grantdocumentlink: grantDocumentLink || null,
  ...
  for (const row of updates) {
    try {
      const { data: entries, error: fetchError } = await supabase
        .from('patents')
        .select('email, patenttype, filingdate, grantingdate, publishingdate')
        .eq('id', row.id);

      if (fetchError || !entries || entries.length === 0) {
        rejectedEntries.push({ id: row.id, reason: "Entry not found" });
        continue;
      }

      const existing = {
        email: entries[0].email,
        patentType: entries[0].patenttype,
        filingDate: entries[0].filingdate,
        grantingDate: entries[0].grantingdate,
        publishingDate: entries[0].publishingdate,
      };

      const dateValidation = validatePatentDates(patentType, filingDate, grantingDate, publishingDate);
      if (!dateValidation.valid) {
        rejectedEntries.push({ id: row.id, reason: dateValidation.message });
        continue;
      }
      ...
      const { error: updateError } = await supabase
        ...
        .update({
          ...
          coapplicants: row.coApplicants ?? undefined,
          patentid: row.patentId ?? undefined,
          patenttitle: row.patentTitle ?? undefined,
          patenttype: patentType ?? undefined,
          approvaltype: row.approvalType ?? undefined,
          filingdate: row.filingDate ? formatDateForMySQL(row.filingDate) : undefined,
          grantingdate: row.grantingDate ? formatDateForMySQL(row.grantingDate) : undefined,
          publishingdate: row.publishingDate ? formatDateForMySQL(row.publishingDate) : undefined,
          documentlink: row.documentLink ?? undefined,
          grantdocumentlink: row.grantDocumentLink ?? undefined,
      ...
      rejectedEntries.push({ id: row.id, reason: err.message });
    }
  }

  await logAction(req.user.userEmail, "BATCH_UPDATE", `Batch update: ${successfulUpdates.length} successful, ${rejectedEntries.length} rejected`);
  ...
  try {
    ...
    const { error: updateError } = await supabase
      ...
      .update({
        ...
        coapplicants: coApplicants ?? undefined,
        patentid: patentId ?? undefined,
        patenttitle: patentTitle ?? undefined,
        patenttype: finalPatentType ?? undefined,
        approvaltype: approvalType ?? undefined,
        filingdate: filingDate ? formatDateForMySQL(filingDate) : undefined,
        grantingdate: grantingDate ? formatDateForMySQL(grantingDate) : undefined,
        publishingdate: publishingDate ? formatDateForMySQL(publishingDate) : undefined,
        documentlink: finalDocumentLink ?? undefined,
        grantdocumentlink: finalGrantDocLink ?? undefined,
  ...
  try {
    ...
    for (const [key, value] of Object.entries(filters)) {
      if (value && allowedColumns.includes(key)) {
        const dbKey = DB_COLUMN_MAP[key] || key;
        query = query.ilike(dbKey, `%${value}%`);
      }
    }

    const dbSortKey = DB_COLUMN_MAP[safeSortKey] || safeSortKey.toLowerCase();
    query = query.order(dbSortKey, { ascending: finalSortDirection === 'asc' });
